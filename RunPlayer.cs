...
public class RunPlayer : MonoBehaviour
{
	...
	{
		Song = song;
		GetComponent<AudioSource>().time = 0;    //從第幾秒開始播放
		GetComponent<AudioSource>().clip = Song.BackgroundTrack; //抓歌曲
		GetComponent<AudioSource>().pitch = 1;   //會改變音高，超過1會加速而且變尖銳，小數值會變慢變低音，畫面會回溯很可怕
